        ...
        if (
            ...
            if isinstance(annotation.slice, ast.Tuple):
                ...
                if len(elts) >= 2:
                    ...
                    sig_elt = elts[1]
                    # Old style: bare string
                    if isinstance(sig_elt, ast.Constant) and isinstance(
                        sig_elt.value, str
                    ):
                    ...
                    if isinstance(sig_elt, ast.Call):
                        if (
                            isinstance(sig_elt.func, ast.Name)
                            and sig_elt.func.id == "DBusSignature"
                        ):
                            if sig_elt.args and isinstance(
                                sig_elt.args[0], ast.Constant
                            ):
            ...
            for item in node.body:
                if isinstance(item, ast.FunctionDef | ast.AsyncFunctionDef):
                    ...
                    if "method" in decorator_names or "dbus_method" in decorator_names:
                        ...
                        return_type = (
                            resolve_type(item.returns) if item.returns else None
                        )
                        ...
                        "signal" in decorator_names or "dbus_signal" in decorator_names
                    ):
                        ...
                        for j in range(i + 1, len(inner)):
                            if inner[j] == "(":
                                ...
                                if depth == 0:
                                    ...
                                    elem_type = dbus_sig_to_cpp_type(
                                        inner[start + 1 : j + 1]
                                    )
...
def generate_interface_xml(
    ...
    for method in sorted(methods, key=lambda m: m.name):
        ...
        for idx, (arg_name, arg_type) in enumerate(method.args):
            lines.append(
                f'      <arg name="{arg_name}" direction="in" type="{arg_type}"/>'
            )
            qt_type = dbus_type_to_qt_type(arg_type)
            if qt_type:
                lines.append(
                    f'      <annotation name="org.qtproject.QtDBus.QtTypeName.In{idx}" value="{qt_type}"/>'
                )
        ...
        if method.return_type:
            ...
            if qt_type:
                lines.append(
                    f'      <annotation name="org.qtproject.QtDBus.QtTypeName.Out0" value="{qt_type}"/>'
                )
    ...
    for prop in sorted(properties, key=lambda p: p.name):
        lines.append(
            f'    <property name="{prop.name}" type="{prop.type}" access="{prop.access}"/>'
        )
    ...
    for signal in sorted(signals, key=lambda s: s.name):
        ...
        for idx, (arg_name, arg_type) in enumerate(signal.args):
            lines.append(
                f'      <arg name="{arg_name}" direction="out" type="{arg_type}"/>'
            )
        lines.append("    </signal>")
